...
create() {
    ...
    client)
        ...
        if [[ $is_full_client ]]; then
            ...
            is_inbounds='inbounds:[{port:30666,listen:"127.0.0.1",protocol:"socks",settings:{udp:true},sniffing:{enabled:true,destOverride:["http","tls"]}}]'
...
get() {
    ...
    dynamic-port) # create random dynamic port
        if [[ $port -ge 60000 ]]; then
            is_dynamic_port_end=$(shuf -i $(($port - 30666))-$port -n1)
            is_dynamic_port_start=$(shuf -i $(($is_dynamic_port_end - 30666))-$is_dynamic_port_end -n1)
        else
            is_dynamic_port_start=$(shuf -i $port-$(($port + 30666)) -n1)
            is_dynamic_port_end=$(shuf -i $is_dynamic_port_start-$(($is_dynamic_port_start + 30666)) -n1)
        fi
...
info() {
    ...
    tcp | kcp | quic)
        ...
        is_vmess_url=$(jq -c '{v:2,ps:"'QDwbd-${net}-$is_addr'",add:"'$is_addr'",port:"'$port'",id:"'$uuid'",aid:"0",net:"'$net'",type:"'$header_type'",path:"'$kcp_seed'"}' <<<{})
        ...
        if [[ $is_reality ]]; then
            ...
            is_url="$is_protocol://$uuid@$ip:$port?encryption=none&security=reality&flow=xtls-rprx-vision&type=tcp&sni=$is_servername&pbk=$is_public_key&fp=chrome#QDwbd-$net-$is_addr"
        fi
        ;;
    ss)
    ...
    ws | h2 | grpc | xhttp)
        ...
        [[ $is_protocol == 'vmess' ]] && {
            ...
            is_url="$is_protocol://$uuid@$host:$is_https_port?encryption=none&security=tls&type=$net&host=$host&${is_url_path}=$(sed 's#/#%2F#g' <<<$path)#QDwbd-$net-$host"
        }
